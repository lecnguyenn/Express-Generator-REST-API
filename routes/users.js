...
router.post('/login', cors.corsWithOption, (req,res, next) =>{
  ...
  passport.authenticate('local', (err, user, info) =>{
    ...
    if(!user){
      res.statusCode = 401;
      res.setHeader('Content-Type', 'application/json');
      res.json({success: false, status:' Login Unsucess', err: info})
    }
    req.logIn(user, (err) =>{
      if(err){
        res.statusCode = 401;
        res.setHeader('Content-Type', 'application/json');
        res.json({success: false, status: 'Login Unsucess', err:'Could not login in user' })
      }
      var token = authenticate.getToken({_id: req.user._id});
      res.statusCode = 200;
      res.setHeader('Content-Type', 'application/json');
      res.json({success: true, status: 'Login Success', token: token})
    });
  })(req, res,next)
});
